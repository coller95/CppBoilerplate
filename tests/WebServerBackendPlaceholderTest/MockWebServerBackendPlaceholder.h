...
/**
 * Mock implementation of IWebServer with Placeholder-specific methods for testing
 */
class MockWebServerBackendPlaceholder : public webserver::IWebServer {
public:
    // IWebServer methods
    ...
    // Placeholder-specific methods (no longer from interface, but can still be mocked)
    MOCK_METHOD(void, setSimulatedLatency, (int milliseconds));
    MOCK_METHOD(size_t, getHandlerCount, (), (const));
    MOCK_METHOD(void, clearAllHandlers, ());
};
